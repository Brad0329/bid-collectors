...
|---|---|---|---|
| 2026-09-26 | **(변경안, Phase 011) — 원칙 ② 표준 필드 파생 제거 + d2b url 상세 화면.** 조사 2026-09-26: 이 패키지 위반 전수(알려진 11 + 새 약 20) · bidwatch 영향표(읽기만) · d2b 사이트 실측(`scripts/_tmp/d2b_url/`). **bidwatch 제약**(영향표): status·organization·url·detail_url이 None이면 응답 500·목록 누락(`schemas/notice.py`가 `str`) → 이 필드들은 None 대신 `""`/기존 값 유지 / `content`가 비어 있지 않으면 상세 조회를 건너뛴다(`services/notice.py:84`) → content를 합성으로 채우지 않는다 / bidwatch는 `only_ongoing`을 넘기지 않는다 / `closed`에 의존하는 코드 없음, D-day는 end_date로 자체 계산, 취소는 이미 extra 원문으로 판정(F-017). **A. 결정 필요** — ① status ② 금액·분류·본문의 대체·합성 ③ K-Startup `only_ongoing` 기본값 ④ 원칙 ②의 예외 범위(단일 기관 API의 기관명 상수·ID로 URL 조립·첨부 이름 자리표시자·d2b API 제약 날짜 조건) — 아래 '사용자 확인' 칸에 결과. **B. 기본안(이의 없으면 적용)**: region — 나라장터 입찰 용역·물품 `""`(수요기관명은 지역이 아니다)·공사 `cnstrtsiteRgnNm`(원문 지역 필드), 기업마당 `""`(소관기관), 계약 `""`(관할 구분) / 중소벤처 category `""`(`writerPosition`은 작성 부서) / K-Startup organization = `pbanc_ntrp_nm`만(`sprv_inst` 기관 유형·"창업진흥원" 폴백 제거, 없으면 `""`) / content 500자 절단 해제(K-Startup·중소벤처) / end_date — 보조금24 `신청기한`이 기간이면 **끝** 날짜(기간 분해는 형식 통일), 계약 `cntrctPrd`(계약기간 — 마감일이 아님) → None / 낙찰 start_date 폴백(`rlOpengDt`) 제거. GenericScraper status(게시일 판정)는 2026-09-25 "안 함" 유지(bidwatch가 배지를 숨긴다). **C. d2b url**(bidwatch 요청서 `bid-collectors_d2b_detail_url.md`): 종류별 상세 화면 GET 주소 — 요청서 틀 + **메뉴 파라미터 필수**(없으면 첫 클릭 500: 국내경쟁 `lv2Divs=1&pageDivs=G1&bid_divs=bid`·시설경쟁 `lv2Divs=1&pageDivs=E1`·국내수의 `pageDivs=G`·시설수의 `pageDivs=E`·국외경쟁 추가 없음). 실측 418행(7일 전량) 쿠키 없이: 412 열림(건명+공고번호 본문 확인), **6건 = `cntrctMth` 지명경쟁 → 로그인 필요**(국내경쟁 2·시설경쟁 4) → 목록 화면 폴백(시설 `/peb/bid/announceList.do?key=41` · 국내 물품 `/pdb/bid/goodsBidAnnounceList.do?key=13` · 용역 `/psb/bid/serviceBidAnnounceList.do?key=32`, `busiDivs`로 고름 — cold 200·로그인 문구 없음, 그리드 렌더링은 미확인). 요청서 §3-② iemNo `001` 행은 `pageDivs`로 열린다(원인은 세션 문맥, 폴백 불요). 주소 필드 결측 0/418 — 결측이면 첫 화면 + errors에 "상세 주소 불가 N건". 차수 1·2는 서로 다른 화면(11/11). 비공식 경로 — interface.md에 명시. `detail_url`은 `""` 유지. **버전**: 필드 제거·타입 변경·bid_no 변경이 없으면 minor 1.6.0(값 의미 변경은 handover에 전부), status 제거를 고르면 major 2.0.0 + 브랜치 운영 | ✅ 2026-09-26 — ① status: 출처가 명시한 취소를 `cancelled`로(bidwatch F-017과 같은 키·값: 나라장터 `ntceKindNm`·LH `bidKind`·d2b 경쟁 `pblancSe` = "취소공고", 가스 `CANCEL_YN` = "취소", d2b 수의 `progrsSttus` = "공개협상취소"), ongoing/closed의 마감일 판정·낙찰 `closed`는 **원칙 ②의 명시적 예외(편의 계산값)** ② 원문 한 필드만(기록안대로) ③ `only_ongoing` 기본 False + handover → **정정(같은 날, 실측 후 사용자 재확인): 기본 True 유지 = 원칙 ②의 명시적 예외** — K-Startup에 등록일 필드가 없고, 서버 날짜 필터 `cond[pbanc_rcpt_bgng_dt::GTE]`는 matchCount에만 먹고 데이터엔 안 먹는다(100건 중 98건 위반), 목록은 등록 순번 순이라 접수시작일로 조기 종료 불가 — 필터가 없으면 30,168건(302페이지)(`scripts/_tmp/p011_kstartup*.py`) ④ 예외 = **ID로 URL 조립·URL 폴백, 첨부 이름 자리표시자**만. **예외 아님 → 제거**: 단일 기관 기관명 상수(LH·가스·수자원·중소벤처·d2b 국외·GenericScraper `config.name` → `""`) · d2b 날짜 조건(API가 조건 없이는 0건·무시라 없앨 수 없다 → **범위를 넓혀 `days` 밖으로 거르지 않게**: 수의 2종 견적서 마감 `오늘-days ~ +1년`, 국외 개찰일 `-1년 ~ +1년` — 실측 후 확정) · B 기본안 그대로. minor 1.6.0 | |
| 2026-09-26 | **v1.5.0(예정, Phase 010) — 기관 수집기 4종 `fetch_detail(bid_no)` 구현**(종전 None). 이유: bidwatch 요청서 `bidwatch/docs/requests/bid-collectors_institution_fetch_detail.md`(팝업 정보가 기관 화면보다 적다 — 수자원 목록 필드 12개). 계약은 알리오(v1.3.0)와 같게: 반환 dict = 상세 원천의 비어 있지 않은 필드 전부·원래 이름 + `attachments: [{"name","url"}]`(없으면 `[]`) + `content`. **실패·없는 공고는 예외**(HTTP 오류·성공 코드 아님·형식 이상·bid_no 형식 불일치 → 요청 없이 `ValueError`). 조사 2026-09-26(표본 수자원 24·가스 26·LH 23·d2b 13+14, 원문 `scripts/_tmp/{kwater,kogas,lh,d2b}_dtl/`). **기관별**: ① 수자원 — 사이트 내부 JSON `POST ebid.kwater.or.kr/.../selectBidPblancDtl.do`(비공식, 인증 없음) 1회. `data.tndrPblanc` 원문 키 평탄화 + `data`의 나머지 비어 있지 않은 필드 원문 그대로(`tndrPrgsOrdrList` 입찰 일정·`atchflList` 포함 — 구현 시 원칙 ①대로 골라 담지 않음, 키 충돌이면 예외). 첨부 url = `sc/file/downloadAtchFileOne.do?xmlValue={"atchflId","fileSeq"}`(3건 실제 받음), name = `docFileNm`. 없는 번호도 `success` + `tndrPblanc: null`이라 그것으로 판정. ② d2b — 공식 API 상세 오퍼레이션 5종(시설수의 `getFcltyOthbcVltrnNtatPlanDetail` 포함). 국내·국외경쟁은 bid_no에서 파라미터 복원 → 1회. **시설경쟁(`pblancSeCode`)·국내수의(`iemNo`·`ntatPlanDate`)·시설수의(`ntatPlanDate`)는 bid_no에 없는 값이 필수** → 목록 오퍼레이션을 bid_no 키로 1회 조회해 값을 얻고 상세 1회(총 2회, 목록 한도를 `collect`와 나눠 씀). `item` 원문 필드 평탄화(반복·중첩 없음, `areaLmttList` 등 `^` 구분 문자열은 원문 그대로). 첨부·본문 필드가 없어 `attachments: []`·`content: ""`. 없는 번호·틀린 파라미터는 `resultCode 00` + 빈 body라 원인 구분 불가 — 예외 메시지에 그렇게 적는다. ③ 가스 — 사이트 HTML `bid_detail_view_notice.jsp?notice_code=&bid_code=001&round=01`(비공식, 001/01 외엔 400, 재공고는 새 notice_code) 1회. 키 = **화면 항목명**(공백 정리, 표본 26건 페이지 안 중복 0 — 중복이 나오면 list로 모은다) + 이름 없는 상단 진행 단계·안내 문구는 `진행상태`·`진행안내`, 품목표는 `품목내역`(열 제목을 키로 한 list[dict]), attachments = 페이지의 내려받기 링크 전부(공고 첨부·표준 계약조건·구매요청 — 구현 시 원칙 ①대로 골라 담지 않음, url은 `href` 절대 주소 — seq가 이어지지 않음). "정보가 존재하지 않습니다" alert·공고번호 칸 불일치·핵심 항목 없음 → 예외. ④ LH — 사이트 HTML(비공식). bid_no에 업무 구분·차수가 없어 **검색 `BidMasterListCmd` 1회로 최신 차수·업무 코드를 얻고 상세 1회(총 2회)**. 틀린 cmd도 200에 다른 화면 틀이라 추측 시도는 쓰지 않는다. 키 = **`"표 이름/항목명"`**(참가지역1~4가 두 표에 겹쳐 23/23건 충돌), 제목 행이 전부 th인 목록형 표(요구면허·파일정보·공고변경정보)는 `표 이름` → list[dict], 같은 이름의 표가 다시 나오면 `표 이름#2`(시설공사 요구면허 2개 — 구현 시 확인). 첨부 url = `ebid.framework.download.dev?download.filespec=bidinfo&download.filename=&download.savedname=`(1건 실제 받음). 공고번호 칸이 `NNNNNNN - NN`이 아니거나 건명 빈 값 → 예외. **TLS**: `ebid.lh.or.kr` 인증서 체인에 중간 인증서가 빠져 httpx 기본(certifi)으로 검증 실패 — (추천) 중간 인증서(TuringSign RSA Secure CA 2)를 패키지에 동봉해 이 호스트 요청에만 추가 신뢰(새 의존성 없음). **공통**: `content` = 원천에 공고 본문 필드가 없어 넷 다 `""`(가스 `5. 업체제시문`·수자원 `tndrQualfAtpn`은 원문 키로 들어간다 — content로 옮기는 것은 파생). 값은 원문 그대로(`()`·`~`·`-` 같은 빈 표기도 정리하지 않음, 빈 문자열만 제외). 수집 경로(`collect`)는 부르지 않는다. 비공식 3곳은 사이트 개편 시 예외로 깨진다는 사실을 interface.md에. 영향: 시그니처·bid_no 불변, 4종의 반환값 None → dict. `interface.md` §2 갱신(여기 — bidwatch 반영은 handover). minor | ✅ 2026-09-26 (d2b 3종 = 목록 조회 후 상세 · LH TLS = 중간 인증서 동봉 · 나머지 기록안 그대로) | `7587bae`~Phase 010 마감 커밋 (bidwatch 반영은 `docs/handover/v1.5.0.md` §5). `lh_ssl_context()`는 내부 — 공개 export 아님 |
