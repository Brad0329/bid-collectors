...
│   ├── smes.py              중소벤처기업부 사업공고 (계획했던 smes24.py 중소벤처24는 Phase 003에서 안 함 — 같은 데이터)
│   ├── defense.py           방위사업청
│   ├── lh.py                한국토지주택공사
│   ├── kepco.py             한국전력공사
...
| ~~`smes24.py`~~ | 중소기업기술정보진흥원 | 중소벤처24 공고정보 | 중기부 산하 통합공고 | **안 함**(Phase 003) — `smes.py`(중소벤처기업부 사업공고)가 같은 데이터 |
